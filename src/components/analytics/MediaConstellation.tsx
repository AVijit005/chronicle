    ...
    <PremiumGlass className="relative flex flex-col p-4 md:p-5 overflow-hidden min-h-[280px]">
      ...
      {isLoading || constellationData.length === 0 ? (
        ...
        <>
          ...
          <div className="relative z-10 mt-auto grid grid-cols-2 md:grid-cols-5 gap-1.5 md:gap-2 w-full pt-1">
              ...
              {constellationData.map((item) => {
                ...
                return (
                  <motion.div 
                    ...
                    key={item.label}
                    className={`flex flex-col gap-1.5 p-2.5 rounded-xl transition-all duration-500 cursor-pointer border ${isActive ? 'bg-white/10 border-white/10 shadow-lg scale-[1.02]' : 'border-transparent hover:bg-white/[0.04]'}`}
                    onMouseEnter={() => setHovered(item.label)}
                    onMouseLeave={() => setHovered(null)}
                  >
